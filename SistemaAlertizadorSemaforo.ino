...
void loop(){
  // Lee el valor del pot y lo guarda en la variable valorPot
  valorPot=analogRead(pot);

  //Realiza la lectura del estado del boton
  estadoBoton = digitalRead(botonPeatonal);        

  //Se manda a llamar al metodo encargado de realizar todo el proceso de los estados a partir del estado del boton
  estadoSemaforo(estadoBoton);
...
void estadoSemaforo(int estadoBoton){
  
//Si el boton no esta presionado, es decir en el estado low, se indica que el led verde de semaforo de carros y el
  //led rojo del semaforo peatonal siempre esten encendidos.
  //El semaforo de carro siempre debe de estar en verde hasta que se presione el boton 
  if (estadoBoton == LOW){                         
    digitalWrite(ledVerde_carro, HIGH);                
    digitalWrite(ledRojo_peaton, HIGH);         

   //Si se presiona el boton
  }else if(estadoBoton == HIGH){  
    //Espera el tiempo determinado por el potenciometro para volver apresionar el boton y solicitar el paso
    delay(valorPot*100);
